        ...
        if (parameter.in === "body") {
          if (
            ...
            if (parameter.value) {
              ...
              operation.requestBody = OpenApiModelExample.getModelExample(
                parameter.schema["$ref"],
                this.get("apiService").api,
                false
              );
              ...
              if (
                ...
                operation.requestBody =
                  "[" +
                  OpenApiModelExample.getModelExample(
          ...
          if (parameter.schema["$ref"]) {
            ...
            let definition = OpenApiModelExample.getModelDescription(
              parameter.schema["$ref"],
              this.get("apiService").api,
              false
            );
            definition.forEach((element) => {
              element.definitions.forEach((def) => {
                var type = def.type;
                if (
                  type.includes("definitions") &&
                  !allPossibleDefinitions.includes(type)
                ) {
                  allPossibleDefinitions.push(
            ...
            allPossibleDefinitions.forEach((index) => {
              if (!definition.some((element) => index === element.name)) {
                definition = definition.concat(
                  OpenApiModelExample.getModelDescription(
                    `#/definitions/${index}`,
                    this.get("apiService").api,
                    false
                  )
...
*/
    ...
    share() {
      let operation = this.get("operation");
      this.get("shareService").setSharableOperation(
        this.get("operation"),
        operation.requestBody,
        this.get("computedHeaders"),
  ...
  getNotificationTopics: computed(
    "notificationService.getTopicForResource",
    function () {
      let operation = this.get("operation");
